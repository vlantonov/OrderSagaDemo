...
- **CI vuln step no longer fails to install `govulncheck` under Go 1.22.** The step used `go install golang.org/x/vuln/cmd/govulncheck@latest`, but `x/vuln@v1.7.0` raised its `go.mod` `go` directive to `1.25.0`, so `go install` aborted with `requires go >= 1.25.0` under the pinned Go 1.22 toolchain (`GOTOOLCHAIN=local`). Pinned govulncheck to **`@v1.1.4`** — verified as the highest `x/vuln` release whose module `go` directive is `≤ 1.22` (`v1.2.0`–`v1.7.0` all declare `go 1.25.0`) — in the `make vuln` target (`Makefile`), the CI workflow (`.github/workflows/ci.yml`), and the pipeline docs (`docs/ci-cd/pipeline.md`). The vuln step remains non-blocking (`continue-on-error: true`); Go stays at 1.22 (ADR-002).
